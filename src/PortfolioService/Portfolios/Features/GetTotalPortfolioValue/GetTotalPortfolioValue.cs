    ...
    GetTotalPortfolioValueHandler : IQueryHandler<GetTotalPortfolioValue, GetTotalPortfolioValueResponse>
{
    ...
    )
    {
        ...
        foreach (var stock in portfolio.Stocks)
        {
            ...
            var convertedPrice = CalculateConvertedStockPrice(
    ...
    private static decimal CalculateConvertedStockPrice(
        decimal stockPrice,
        string baseCurrency,
    ...
    {
        if (baseCurrency == targetCurrency) return stockPrice;

        if (data == null) throw new Exception("Exchange data cannot be null when converting currencies.");

        var rateToUsd = baseCurrency == "USD" ?
                            1m : // 1 USD = 1 USD  treat the condition where baseCurrency is "USD" as a special case
                            data.quotes["USD" + baseCurrency];

        if (targetCurrency == "USD") return stockPrice / rateToUsd;
